...
export const getById = async (id) => {
  logger.info("*** GET BY ID USER DAO ***");
  try {
...
export const findUserToken = async (user) => {
...
export const hasPassword = async (user) => {
  logger.info("*** HAS PASSWORD USER DAO ***");
  try {
    let userFinded = await User.findOne({
      _id: user._id,
    }).select("security");
    if (!userFinded) throw createError(404, "User not found");
    if (userFinded.security && userFinded.security.hasPassword) {
...
export const changePassword = async (data, user) => {
  global.logger.info("*** CHANGE PASSWORD USER DAO ***");
  try {
    ...
    const validPass = await User.validPassword(user._id, currentPassword);
    if (!validPass) {
...
//   });
//   // Trae todos los holiday con sus usuarios que tienen bloqueo por fecha y hora
...
//   // Extraer los usuarios de schedules y holidays
//   const scheduleUsers = schedules.map((schedule) => schedule.user);
//   const holidayUsers = holidays.map((holiday) => holiday.user);

//   // Combinar los usuarios de schedules y holidays
//   let combinedUsers = [...scheduleUsers, ...holidayUsers];
